...
<script>
  function validateFileSize(input) {
    ...
    const maxSize = 500 * 1024;  // Maximum size in bytes (500 KB)

    if (file) {
      if (file.size > maxSize) {
        alert('The file size must not exceed 500 KB.');
...
<script>
    $(document).on("change","#parentlocation",function(){
        var parent1 = $(this).val();
        var data = {
              parent: parent1,
            };
            ...
            $.ajax({
              ...
              success: function(response) {
                // Handle success
                $("#prop_location").html('<option>Choose</option>'+response);
